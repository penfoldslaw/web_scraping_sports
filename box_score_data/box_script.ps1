...
$scriptPath = Join-Path -Path $scriptDir "box_scraper_processor.py"  # Replace with your actual script path
$scriptPath_parser = Join-Path -Path $scriptDir "box_parser_processor.py"
...
if ($process_parser) {
    ...
    Write-Host "box Parser script has completed." -ForegroundColor Cyan
} else {
