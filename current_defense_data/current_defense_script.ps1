...
$scriptPath = Join-Path -Path $scriptDir "current_defense_scraper_processor.py"  # Replace with your actual script path
$scriptPath_parser = Join-Path -Path $scriptDir "current_defense_parser_processor.py"
...
if ($process_parser) {
    ...
    Write-Host "defense Parser script has completed." -ForegroundColor Cyan
} else {
