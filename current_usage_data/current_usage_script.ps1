...
$scriptPath = Join-Path -Path $scriptDir "current_usage_scraper_processor.py"  # Replace with your actual script path
$scriptPath_parser = Join-Path -Path $scriptDir "current_usage_parser_processor.py"
...
if ($process_parser) {
    ...
    Write-Host "Usage Parser script has completed." -ForegroundColor Cyan
} else {
