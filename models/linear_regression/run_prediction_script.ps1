...
$scriptPath = Join-Path -Path $scriptDir "run_predictions.py"  # Replace with your actual script path
...
if ($process) {
    Write-Host "run_prediction is running... (PID: $($process.Id))" -ForegroundColor Yellow
    $process | Wait-Process
    Write-Host "run_prediction has completed." -ForegroundColor Cyan
} else {
    Write-Host "Failed to start the run_prediction script check log." -ForegroundColor Red
}
